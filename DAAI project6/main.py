...
from models.resnet import ASHBaseResNet18
from models.resnet import ASHDAResNet18
...
def train(model, data):
    ...
    print(f"CUDA AVALABEL: {torch.cuda.is_available()}", flush=True)
    # Optimization loop
    for epoch in range(cur_epoch, CONFIG.epochs):
        ...
        for batch_idx, batch in enumerate(tqdm(data['train'])):
            ...
            with torch.autocast(device_type=CONFIG.device, dtype=torch.float16, enabled=True):
                ...
                elif CONFIG.experiment in ['ASHbaseline']:
                    x, y = batch
                    x, y = x.to(CONFIG.device), y.to(CONFIG.device)
                    loss = F.cross_entropy(model(x), y)
                elif CONFIG.experiment in ['ASHDomain_Adaptation']:
                    src_x, src_y, trg_x = batch
                    src_x, src_y, trg_x = src_x.to(CONFIG.device), src_y.to(CONFIG.device), trg_x.to(
                        CONFIG.device)
                    trg_activation_maps = model.get_activation_maps(trg_x)
                    outputs = model(src_x, trg_activation_maps)
                    loss = F.cross_entropy(outputs, src_y)
                    # ######################################################
