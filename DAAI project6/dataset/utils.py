...
class DomainAdaptationDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        y = torch.tensor(y).long()
        x = self.T(x).to(CONFIG.dtype)
        trg_index = torch.randint(len(self.target_examples), size=(1,)).item()
        trg_x, _ = self.target_examples[trg_index]
        trg_x = Image.open(trg_x).convert('RGB')
        trg_x = self.T(trg_x).to(CONFIG.dtype)
        return x, y, trg_x
